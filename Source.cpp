...
int main()
{
	Mat video,temp,cont,vidhsv,vidinrng,vidmorph,labels;   //Empty Matrices for storing images
	...
	{
		cap.read(video);
		temp = imread("C:\\c1.jpg",CV_LOAD_IMAGE_COLOR);//Load Template shapes image
		labels=Mat::zeros(video.size(),CV_8UC1);
		cvtColor(video,vidhsv,CV_BGR2HSV);// Change video from BGR to HSV
		...
		dilate(vidmorph,vidmorph,element);
		erode(vidmorph,vidmorph,element);
		...
		*/
		...
		for(unsigned int k=0;k<cont_live.size();k++)
		{
			...
			//calculate centroid
					...
					double m01 = mymoments.m01; //gives coordinates of ??
					double m10 = mymoments.m10; //gives coordinates of ??
